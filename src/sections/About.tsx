...
export function About() {
  ...
  return (
    <section id="about" className="w-full py-24 lg:py-32 bg-exvia-black">
        ...
        <div ref={sectionRef} className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          ...
          <div className="space-y-8">
            ...
            {aboutConfig.experienceValue && (
              ...
              >
                <span className="text-7xl lg:text-8xl font-black text-exvia-gold leading-none">
                  {aboutConfig.experienceValue}
                </span>
                {aboutConfig.experienceLabel && (
                  <span className="text-sm text-white/60 pb-3">
                    {aboutConfig.experienceLabel}
                  </span>
                )}
              </div>
            ...
            {aboutConfig.stats.length > 0 && (
              <div
                className={cn(
                  'grid grid-cols-3 gap-8 pt-8 border-t border-white/10 transition-all duration-800 ease-out-quart',
                  sectionVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
                )}
              ...
              >
                {aboutConfig.stats.map((stat, index) => (
                  <div key={index}>
                    <span className="block text-3xl font-semibold text-exvia-gold">{stat.value}</span>
